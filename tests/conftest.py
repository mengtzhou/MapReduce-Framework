...
def wait_for_server_ready(process, port):
    ...
    for _ in range(10*TIMEOUT):
        ...
        if process.poll() is not None:
            raise Exception(f"Premature exit: {process}")
        time.sleep(0.1)
    raise Exception(f"Failed to start: {process}")
...
def thread_safe_mocks():
    ...
    We have made an issue on the CPython repo to fix this problem (INSERT LINK
    HERE). This fixture can be removed after the upstream fix is adopted.
    ...
    # Sanity check for a private attribute we're about to add
    assert not hasattr(unittest.mock.NonCallableMock, "_lock"), (
        "NonCallableMock already has a _lock attribute. Reconsider the "
        "implementation of the thread_safe_mocks pytest fixture."
    )
